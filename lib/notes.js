...
function getNotes (nidList) {
	...
	for (var i=0; i<nidList.length; i++) {
		...
		if (n !== null) {
			...
			var note = {nid: n.nid, uid: n.uid, cid: n.cid, date: n.date, message: n.message, ques: n.ques, reply: n.reply, convo: n.convo, mentions: n.mentions, hashtags: n.hashtags, name: user.name, username: user.username, code: course.code};
...
*/
function createNote (uid, msg, cid, ques) {
	var user = users.getInfoByUID(uid);
	var course = courses.getInfoByCID(cid);
	
	//add note to notes list
	//reply = null, convo = null, mentions and hashtags = [] for now
	...
	var newNote = new Note (notedb.length, uid, cid, new Date(), msg, ques, null, null, [], []);
	notedb.push(newNote);
	...
	console.log("note.ques here --- " + note.ques);
